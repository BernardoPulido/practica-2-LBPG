...
exports.getLibros = function(req, res, next){
  Libros.find().populate('autor').exec(function (err, libros) {
      if(err){
...
exports.addLibro = function(req, res, next){
    ...
    libro.save(function (err, libro) {
        ...
        Libros.findById(libro._id).populate('autor').exec(function (err, libro) {
            if(err) return res.status(500).jsonp({error:'500', descrip:err.message});
...
exports.getById = function(req, res, next){
    Libros.findById(req.params.id).populate('autor').exec(function (err, libro) {
        if(err){
...
exports.deleteLibro = function(req, res, next){
    ...
    Libros.findByIdAndRemove(req.params.id).populate('autor').exec(function (err, libro) {
        if(err){
